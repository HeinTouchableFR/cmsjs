...
export default function Index({items, errors}) {
    ...
    return (
        <>
            ...
            <Header>
                <Content titre="Attributs" icon="fa-cubes" url={url}>
                    {errors}
                    <table className={styles.table + " " + styles.tableStriped}>
                        <thead className={styles.thead}>
                            <tr>
                                ...
                                <th className={styles.th} scope="col">Nombre de valeur(s)</th>
                                <th className={styles.th} scope="col">Actions</th>
                            </tr>
                        </thead>
                        <tbody className={styles.tbody}>
                        {items && items.map(item =>
                            <Item item={item} url={url} key={item._id} handleDelete={open}/>
                        )}
                        </tbody>
                    </table>
                    <Confirm
...
export async function getServerSideProps() {
    ...
    await axios.get(process.env.URL + '/api/attributs')
        .then(res => {
            items = res.data.data
        })
        .catch((error) => {
