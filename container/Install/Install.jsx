...
export default function Install() {
    ...
    const [step, setStep] = useState('checkup');
    ...
    const handleChangeEnvironmentState = (data) => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        setLoading(true);
        const resIndex = await fetch('/api/install', {
            method: 'POST',
            headers: {
        ...
        const { success, data } = await resIndex.json();
        if (success) {
            const resSettings = await fetch('/api/install/settings', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const handleSubmitSocials = async (e) => {
        e.preventDefault();
        setLoading(true);
        const resInstall = await fetch('/api/install/socials', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
                            ...
                            step === 'checkup' && (
                                <div className={`${styles.body}`}>
                                    <div className={`${styles.title}`}>
                                        <h1>
                                            {intl.formatMessage({
                                                id: 'checkup', defaultMessage: 'Checkup',
                                            })}
                                        </h1>
                                        <DarkModeButton />
                                    </div>
                                    <Checkup
                                        ...
                                        handleChangeEnvironmentState={handleChangeEnvironmentState}
                                    />
                                    <Button
                                        label={intl.formatMessage({
                                            id: 'install.process', defaultMessage: 'Proceed to installation',
                                        })}
                                        type='button'
                            ...
                            step === 'socials' && (
                                <div className={`${styles.body}`}>
                                    ...
                                    <form onSubmit={handleSubmitSocials}>
                                        ...
                                        <Button
                                            label={intl.formatMessage({
                                                id: 'install.validate', defaultMessage: 'Validate',
                                            })}
                                            loading={loading}
                                            type='submit'
                                        />
